...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  try {
    ...
    // No mode pill rendered (modeLabel only returns "EXIT" or "SHELL"; neither applies here)
    const main = statusItems[0]
    const spinner = main.getChildren()[0]
    const model = statusItems[1]
...
test("direct footer mode label keeps left padding without a status pill", async () => {
  ...
  try {
    ...
    // No mode pill (modeLabel only returns "EXIT"/"SHELL") — content starts with padding from the main box
    expect(statusline?.startsWith(" ")).toBe(true)
  } finally {
    app.cleanup()
  }
})
