...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openaxe.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openaxe.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openaxe.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openaxe.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openaxe.json", "openaxe.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openaxe.json"),
        path.join(project, "openaxe.jsonc"),
      ])
    })
  })
  ...
  describe("mimeType()", () => {
    ...
    test("returns MIME type for TypeScript (or video/mp2t due to extension conflict)", async () => {
      ...
      // Bun may append charset (e.g. "text/javascript;charset=utf-8"), so use includes()
      expect(
        mime === "video/mp2t" ||
