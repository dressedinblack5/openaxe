...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .openaxe/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const openaxe = path.join(test.directory, ".openaxe")
      const tool = path.join(openaxe, "tool")
  ...
  it.instance("ignores non-tool exports in .openaxe/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".openaxe", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools from .openaxe/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const openaxe = path.join(test.directory, ".openaxe")
      const tools = path.join(openaxe, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".openaxe", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const openaxe = path.join(test.directory, ".openaxe")
        const customTools = path.join(openaxe, "tools")
        const plugin = path.join(openaxe, "node_modules", "@opencode-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(openaxe, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".openaxe", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(openaxe, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(openaxe, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(openaxe, "node_modules", "cowsay")
