...
// openaxe.json should look like is often wrong, and openaxe hard-fails on
// invalid config, so users hit cryptic startup errors. Loading this skill
// when the model is asked to touch openaxe's own config files gives it the
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make(
      Effect.fn("Skill.state")(function* () {
        ...
        s.skills[CUSTOMIZE_OPENAXE_SKILL_NAME] = {
          name: CUSTOMIZE_OPENAXE_SKILL_NAME,
          description: CUSTOMIZE_OPENAXE_SKILL_DESCRIPTION,
          location: "<built-in>",
          content: CUSTOMIZE_OPENAXE_SKILL_BODY,
        }
    ...
    const require = Effect.fn("Skill.require")(function* (name: string) {
      ...
      if (info) return info
      return yield* new NotFoundError({ name, available: Object.keys(s.skills).sort() })
    })
    ...
    const available = Effect.fn("Skill.available")(function* (agent?: Agent.Info) {
      const s = yield* InstanceState.get(state)
      const list = Object.values(s.skills).sort((a, b) => a.name.localeCompare(b.name))
