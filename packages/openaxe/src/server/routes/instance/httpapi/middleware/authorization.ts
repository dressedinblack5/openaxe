...
function credentialFromRequest(request: HttpServerRequest.HttpServerRequest) {
  return credentialFromURL(request)
}

function credentialFromURL(request: HttpServerRequest.HttpServerRequest) {
  // auth_token query param removed - only header-based auth supported
  ...
  Effect.gen(function* () {
    ...
    return (effect) =>
      Effect.gen(function* () {
        ...
        return yield* credentialFromURL(request).pipe(
...
export const ptyConnectAuthorizationLayer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return PtyConnectAuthorization.of((effect) =>
      Effect.gen(function* () {
        const request = yield* HttpServerRequest.HttpServerRequest
        // Header-only ticket validation - skip Basic Auth if ticket header present
        if (request.headers[PTY_CONNECT_TOKEN_HEADER] !== undefined) return yield* effect
        return yield* credentialFromURL(request).pipe(
          Effect.flatMap((credential) => validateCredential(effect, credential, config)),
        )
      }),
