...
export function userAgent(client = "cli") {
  return `openaxe/${InstallationChannel}/${InstallationVersion}/${client}`
}
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "dressedinblack5/tap/openaxe"])
      if (tapFormula.includes("openaxe")) return "dressedinblack5/tap/openaxe"
      const coreFormula = yield* text(["brew", "list", "--formula", "openaxe"])
      if (coreFormula.includes("openaxe")) return "openaxe"
      return "openaxe"
    })
    ...
    const upgradeCurl = Effect.fnUntraced(
      function* (target: string) {
        const response = yield* httpOk.execute(HttpClientRequest.get("https://raw.githubusercontent.com/dressedinblack5/openaxe/main/install"))
    ...
    const result: Interface = {
      ...
      method: Effect.fn("Installation.method")(function* () {
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          { name: "brew", command: () => text(["brew", "list", "--formula", "openaxe"]) },
          { name: "scoop", command: () => text(["scoop", "list", "openaxe"]) },
          { name: "choco", command: () => text(["choco", "list", "--limit-output", "openaxe"]) },
        ]
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "dressedinblack5/tap"], { env })
              if (tap.code !== 0) {
                upgradeResult = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "dressedinblack5/tap"])
              const dir = repo.trim()
              if (dir) {
