...
const cli = yargs(args)
  ...
  .command(
    lazyCommand("mcp", "manage MCP (Model Context Protocol) servers", undefined, () =>
  ...
  .command(
    lazyCommand("attach <url>", "attach to a running openaxe server", undefined, () =>
      import("./cli/cmd/attach").then((m) => m.AttachCommand),
    ),
  )
  .command(
    lazyCommand("run [message..]", "run openaxe with a message", undefined, () =>
      import("./cli/cmd/run").then((m) => m.RunCommand),
    ),
  )
  .command(lazyCommand("generate", false, undefined, () => import("./cli/cmd/generate").then((m) => m.GenerateCommand)))
  .command(
    lazyCommand("debug", "debugging and troubleshooting tools", undefined, () =>
      import("./cli/cmd/debug").then((m) => m.DebugCommand),
    ),
  )
  .command(lazyCommand("console", false, undefined, () => import("./cli/cmd/account").then((m) => m.ConsoleCommand)))
  .command(
    lazyCommand("providers", "manage AI providers and credentials", ["auth"], () =>
      import("./cli/cmd/providers").then((m) => m.ProvidersCommand),
    ),
  )
  .command(
    lazyCommand("agent", "manage agents", undefined, () => import("./cli/cmd/agent").then((m) => m.AgentCommand)),
  )
  .command(
    lazyCommand("upgrade [target]", "upgrade openaxe to the latest or a specific version", undefined, () =>
      import("./cli/cmd/upgrade").then((m) => m.UpgradeCommand),
    ),
  )
  .command(
    lazyCommand("uninstall", "uninstall openaxe and remove all related files", undefined, () =>
      import("./cli/cmd/uninstall").then((m) => m.UninstallCommand),
    ),
  )
  .command(
    lazyCommand("serve", "starts a headless openaxe server", undefined, () =>
      import("./cli/cmd/serve").then((m) => m.ServeCommand),
    ),
  )
  .command(
    lazyCommand("web", "start openaxe server and open web interface", undefined, () =>
      import("./cli/cmd/web").then((m) => m.WebCommand),
    ),
  )
  .command(
    lazyCommand("models [provider]", "list all available models", undefined, () =>
      import("./cli/cmd/models").then((m) => m.ModelsCommand),
    ),
  )
  .command(
    lazyCommand("stats", "show session statistics", undefined, () =>
  ...
  .command(
    lazyCommand("import <file..>", "import session data", undefined, () =>
      import("./cli/cmd/import").then((m) => m.ImportCommand),
    ),
  )
  .command(
    lazyCommand("github", "manage GitHub agent", undefined, () =>
      import("./cli/cmd/github").then((m) => m.GithubCommand),
    ),
  )
  .command(
    lazyCommand("pr <number>", "fetch and checkout a GitHub PR branch, then run openaxe", undefined, () =>
  ...
  .command(
    lazyCommand("plugin <module>", "install plugin and update configuration", ["plug"], () =>
      import("./cli/cmd/plugin").then((m) => m.PluginCommand),
    ),
  )
  .command(lazyCommand("db", "database tools", undefined, () => import("./cli/cmd/db").then((m) => m.DbCommand)))
  .fail((msg, err) => {
    if (
