    ...
    switch (next.type) {
      ...
      case "queued.prompts": {
        ...
        if (next.type === "catalog") {
          ...
          if (next.commands !== undefined) this.setCommands(next.commands)
        } else if (next.type === "models") {
          this.setProviders(next.providers)
        } else if (next.type === "variants") {
      ...
      case "stream.patch": {
        ...
        if (patch) {
          if (typeof patch.status === "string") this.clearNoticeTimer()
