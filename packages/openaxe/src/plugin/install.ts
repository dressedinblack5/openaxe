...
export type PatchDeps = {
  ...
  files: (dir: string, name: "openaxe" | "tui") => string[]
}
...
const defaultPatchDeps: PatchDeps = {
  ...
  files: (dir, name) => fileInDirectory(dir, name),
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".openaxe")
}

function patchName(kind: Kind): "openaxe" | "tui" {
  if (kind === "server") return "openaxe"
  return "tui"
}
