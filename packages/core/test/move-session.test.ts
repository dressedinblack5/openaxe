...
describe("MoveSession", () => {
  ...
  it.live("moves nested session changes without cleaning unrelated files", () =>
    Effect.gen(function* () {
      ...
      expect(yield* Effect.promise(async () => Bun.file(path.join(sourceDirectory, "untracked.txt")).exists())).toBe(
        false,
      )
      ...
      expect(
        yield* Effect.promise(async () => $`git status --porcelain -- packages/staged.txt`.cwd(source).text()),
      ).toBe("M  packages/staged.txt\n")
      expect(yield* Effect.promise(async () => fs.readFile(path.join(source, "tracked.txt"), "utf8"))).toBe(
        "unrelated\n",
      )
      expect(yield* Effect.promise(async () => fs.readFile(path.join(source, "untracked.txt"), "utf8"))).toBe(
        "unrelated\n",
      )
    }),
  )
