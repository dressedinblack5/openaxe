...
describe("ProjectCopy", () => {
  ...
  it.live("adds a numeric suffix when a copy directory already exists", () =>
    Effect.gen(function* () {
      ...
      expect(created.directory).toBe(target)
      expect(
        yield* Effect.promise(async () => fs.stat(path.join(parent, "copy")).then((item) => item.isDirectory())),
      ).toBe(true)
      expect(
        yield* Effect.promise(async () => fs.stat(path.join(parent, "copy-2")).then((item) => item.isDirectory())),
      ).toBe(true)
