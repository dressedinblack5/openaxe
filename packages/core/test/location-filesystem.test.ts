...
describe("FileSystem", () => {
  it.live("reads text and binary files", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        yield* Effect.promise( async () => fs.writeFile(path.join(directory, "text.txt"), "hello"))
        yield* Effect.promise( async () => fs.writeFile(path.join(directory, "data.bin"), Buffer.from([0, 1, 2])))
  ...
  it.live("lists direct children", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise( async () => fs.writeFile(path.join(directory, "README.md"), "# Test"))
