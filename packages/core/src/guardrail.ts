...
export interface Interface {
  ...
  readonly verifyProject: (files: ReadonlyArray<string>) => Effect.Effect<ReadonlyArray<VerificationResult>>
}
...
function runVerifier(
  ...
  args: string[],
  cwd: string,
): Effect.Effect<{ output: string; exitCode: ExitCode }> {
...
function verifyTypeScript(
  ...
  return runVerifier(spawner, "tsc", ["--noEmit", "--pretty", "false"], dir).pipe(
    ...
    Effect.catch(() =>
      Effect.succeed({ projectType: "typescript" as const, files, diagnostics: [], passed: true, verifier: "tsc --noEmit" }),
    ),
  )
}

function verifyRust(files: string[], spawner: ChildProcessSpawner): Effect.Effect<VerificationResult> {
  const dir = path.dirname(files[0])
  return runVerifier(spawner, "cargo", ["check", "--message-format", "json"], dir).pipe(
    Effect.map(({ output, exitCode }) => ({
    ...
    Effect.catch(() =>
      Effect.succeed({ projectType: "rust" as const, files, diagnostics: [], passed: true, verifier: "cargo check" }),
    ),
  )
}

function verifyPython(files: string[], spawner: ChildProcessSpawner): Effect.Effect<VerificationResult> {
  const dir = path.dirname(files[0])
  return runVerifier(spawner, "ruff", ["check", "--output-format", "json", ...files], dir).pipe(
    Effect.map(({ output, exitCode }) => ({
...
function verifyGo(files: string[], spawner: ChildProcessSpawner): Effect.Effect<VerificationResult> {
  const dir = path.dirname(files[0])
  return runVerifier(spawner, "go", ["vet", "./..."], dir).pipe(
    Effect.map(({ output, exitCode }) => ({
    ...
    Effect.catch(() =>
      Effect.succeed({ projectType: "go" as const, files, diagnostics: [], passed: true, verifier: "go vet" }),
    ),
  )
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const verifyProject = Effect.fn("Guardrail.verifyProject")(function* (files: ReadonlyArray<string>) {
      ...
      return yield* Effect.forEach(
        ...
        (type) => {
          switch (type) {
            case "typescript":
              return verifyTypeScript(groups[type], spawner)
            case "rust":
              return verifyRust(groups[type], spawner)
            case "python":
              return verifyPython(groups[type], spawner)
            case "go":
              return verifyGo(groups[type], spawner)
            default:
    ...
    return Service.of({ verify, verifyProject })
  }),
)
